...
void AWallRunCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("StrafeJumpLeft", IE_DoubleClick, this, &AWallRunCharacter::StrafeJumpLeft);
	PlayerInputComponent->BindAction("StrafeJumpRight", IE_DoubleClick, this, &AWallRunCharacter::StrafeJumpRight);
...
void AWallRunCharacter::GetWallRunSideAndDirection(const FVector& HitNormal, EWallRunSide& OutSide, FVector& OutDirection) const
{
	...
	//	GEngine->AddOnScreenDebugMessage(1, 1.0f, FColor::Green, TEXT("Capsule hit! Side LEFT")); // delete later
	}
	else
	{
		OutSide = EWallRunSide::Right;
		OutDirection = FVector::CrossProduct(FVector::UpVector, HitNormal).GetSafeNormal();
	//	GEngine->AddOnScreenDebugMessage(1, 1.0f, FColor::Blue, TEXT("Capsule hit! Side RIGHT")); // delete later
	}
}
...
{
	GEngine->AddOnScreenDebugMessage(1, 1.0f, FColor::Green, TEXT("Wall run started"));
...
{
	GEngine->AddOnScreenDebugMessage(1, 1.0f, FColor::Red, TEXT("Wall run ended"));
...
void AWallRunCharacter::StrafeJumpLeft()
{
	if (GetMovementComponent()->IsFalling())
	{
		LaunchCharacter(-GetActorRightVector() * 500.0f, false, false);
	}
	else
	{
		LaunchCharacter(-GetActorRightVector() * 1000.0f, false, false);
	}
}

void AWallRunCharacter::StrafeJumpRight()
{
	if (GetMovementComponent()->IsFalling())
	{ 
		LaunchCharacter(GetActorRightVector() * 500.0f, false, false);
